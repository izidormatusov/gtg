...
def task_from_element(task, element: etree.Element):
    ...
    try:
        ...
        if content:
            ...
            etree.strip_tags(content, 'content')
            content = etree.tostring(content)

            task.set_text(content.decode())
...
def task_to_element(task) -> etree.Element:
    ...
    if text:
        ...
        content.text = etree.tostring(text)
...
def get_backup_name(filepath: str, i: int) -> str:
    ...
    backup_file = f"{filename}.bak.{i}"

    return os.path.join(dirname, 'backup', backup_file)


def get_xml_tree(filepath: str) -> etree.Element:
...
def open_file(xml_path: str, root_tag: str) -> etree.Element:
